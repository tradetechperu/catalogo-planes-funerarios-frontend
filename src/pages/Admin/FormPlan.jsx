...
export default function FormPlan() {
  ...
  useEffect(() => {
    (async () => {
      if (isNew) return;
      try {
        const list = await api(`/api/admin/planes`, { method: "GET" });
...
const uploadGallery = async (files) => {
  ...
  const setCaptionBySrc = (src, titulo) => {
    setForm((p) => {
      if (p.fotoPrincipal?.src === src) return { ...p, fotoPrincipal: { ...p.fotoPrincipal, titulo } };
      const galeria = (Array.isArray(p.galeria) ? p.galeria : []).map((g) => (g.src === src ? { ...g, titulo } : g));
      return { ...p, galeria };
    });
  };
  ...
  const setPrincipal = (src) => {
    setForm((p) => {
      ...
      return { ...p, fotoPrincipal: { src, titulo: caption }, galeria: newGal };
    });
  };
  ...
  const removeFoto = (src) => {
    setForm((p) => {
      ...
      return { ...p, fotoPrincipal: newPrincipal, galeria: newGaleria };
    });
  };
  ...
  const onSave = async () => {
    ...
    try {
      const incluye = form.incluyeText.split("\n").map((x) => x.trim()).filter(Boolean);
      const ataudes = form.ataudesText.split(",").map((x) => x.trim()).filter(Boolean);
      ...
      const payload = {
        ...
        galeria: (Array.isArray(form.galeria) ? form.galeria : [])
          .map((g) => ({ src: String(g.src || "").trim(), titulo: String(g.titulo || "").trim() }))
          .filter((g) => g.src),
      };
      ...
      if (isNew) await api("/api/admin/planes", { method: "POST", body: JSON.stringify(payload) });
      else await api(`/api/admin/planes/${id}`, { method: "PUT", body: JSON.stringify(payload) });
  ...
  const chipsIncluye = form.incluyeText.split("\n").map((x) => x.trim()).filter(Boolean).slice(0, 8);
  const chipsAtaudes = form.ataudesText.split(",").map((x) => x.trim()).filter(Boolean).slice(0, 8);
    ...
    <Box sx={{ minHeight: "100vh", bgcolor: "#fafafa", py: 4 }}>
      <Container maxWidth="md">
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
          ...
          <Button variant="outlined" onClick={() => navigate("/admin/planes")}>Volver</Button>
        </Stack>
        ...
        <Card sx={{ borderRadius: 3 }}>
          <CardContent>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <TextField label="Descripción corta" value={form.descripcionCorta} onChange={set("descripcionCorta")} fullWidth />
              </Grid>
              ...
              <Grid item xs={12}>
                ...
                <Stack direction="row" spacing={1} sx={{ mt: 1 }} flexWrap="wrap" useFlexGap>
                  {chipsIncluye.map((c, idx) => <Chip key={idx} label={c} size="small" />)}
                </Stack>
              </Grid>
              ...
              <Grid item xs={12}>
                ...
                />
                <Stack direction="row" spacing={1} sx={{ mt: 1 }} flexWrap="wrap" useFlexGap>
                  {chipsAtaudes.map((c, idx) => <Chip key={idx} label={c} size="small" />)}
                </Stack>
              </Grid>
              ...
              <Grid item xs={12} sm={6}>
                <FormControlLabel
                  control={
                    <Switch checked={Boolean(form.activo)} onChange={(e) => setForm((p) => ({ ...p, activo: e.target.checked }))} />
                  }
              ...
              <Grid item xs={12}>
                <TextField label="Tags (separados por coma)" value={form.tagsText} onChange={set("tagsText")} fullWidth />
              </Grid>
                    ...
                    onChange={async (e) => {
                      try {
                        ...
                        setForm((p) => {
                          ...
                          const principalTitulo = (principalSrc === p.fotoPrincipal?.src ? p.fotoPrincipal?.titulo : "") || "";
                  ...
                  {allFotos.map((src, idx) => {
                    ...
                    return (
                      <Grid item xs={12} sm={6} md={4} key={src + idx}>
                        <Box sx={{ borderRadius: 2, overflow: "hidden", border: isPrincipal ? "2px solid #000" : "1px solid #eee", bgcolor: "white" }}>
                          <img src={viewSrc} alt={`foto-${idx}`} style={{ width: "100%", height: 190, objectFit: "cover", display: "block" }} />
